        ...
        <form action={createSponsor} className="rounded-[2rem] border border-border bg-background p-8">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <AdminField label="Name">
              <input name="name" required disabled={!canEdit} className={adminFieldClassName} />
            </AdminField>
            <AdminField label="Tier">
              <select name="tier" defaultValue="community" disabled={!canEdit} className={adminFieldClassName}>
            ...
            <AdminField label="Link">
              <input type="url" name="link" disabled={!canEdit} className={adminFieldClassName} />
            </AdminField>
            <AdminField label="Sort order">
              <input type="number" name="sortOrder" defaultValue={sponsors.length + 1} disabled={!canEdit} className={adminFieldClassName} />
            </AdminField>
          </div>
          <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_240px]">
            <AdminField label="Tagline">
              <textarea name="tagline" rows={3} disabled={!canEdit} className={`${adminFieldClassName} min-h-24 py-3`} />
            </AdminField>
            <AdminField label="Logo file">
              <input type="file" name="logo" accept="image/*" disabled={!canEdit} className={`${adminFieldClassName} h-11 py-2`} />
            </AdminField>
          </div>
            ...
            <article key={sponsor.id} className="rounded-[1.75rem] border border-border bg-background p-6">
              <form action={updateSponsor}>
                ...
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_180px_200px_140px]">
                  <AdminField label="Name">
                    <input name="name" defaultValue={sponsor.name} required disabled={!canEdit} className={adminFieldClassName} />
                  </AdminField>
                  <AdminField label="Tier">
                    <select name="tier" defaultValue={sponsor.tier} disabled={!canEdit} className={adminFieldClassName}>
                      {sponsorTiers.map((tier) => (
                        <option key={tier} value={tier}>
                          {tier}
                        </option>
                  ...
                  <AdminField label="Link">
                    <input type="url" name="link" defaultValue={sponsor.link} disabled={!canEdit} className={adminFieldClassName} />
                  </AdminField>
                  <AdminField label="Sort order">
                    <input type="number" name="sortOrder" defaultValue={sponsor.sortOrder} disabled={!canEdit} className={adminFieldClassName} />
                  </AdminField>
                </div>
                <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_240px]">
                  <AdminField label="Tagline">
                    <textarea name="tagline" defaultValue={sponsor.tagline} rows={3} disabled={!canEdit} className={`${adminFieldClassName} min-h-24 py-3`} />
                  </AdminField>
                  <AdminField label="Replace logo">
